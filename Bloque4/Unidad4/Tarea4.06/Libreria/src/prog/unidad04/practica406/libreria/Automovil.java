...
public class Automovil extends Vehiculo implements ConvertibleATexto{
  ...
  public Automovil(String matricula, Fecha fechaMatriculacion, String color, int plazas) {
    ...
    if (compruebaColor(color) && compruebaPlazas(plazas)) {
    	this.color = color;
    	this.plazas = plazas;
    } else {
    	// Si es incorrecto, decrezco el contador de vehículos matriculados. 
        // Por definición del constructor Vehiculo, se suma una vez se llama al super. 
    	// Es necesario volver a quitarlo llegados aquí.
   ...
   * @return true si es correcto, false si no lo es
   */
  private boolean compruebaPlazas(int plazas) {
