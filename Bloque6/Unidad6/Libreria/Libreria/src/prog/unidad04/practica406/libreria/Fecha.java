...
public class Fecha  implements ConvertibleATexto{
  ...
  public Fecha(int dia, int mes, int anyo){
    // Si este método se ejecuta sin errores, la fecha es correcta. Sino, se lanza un error IllegalArgumentException
   ...
   */
  private void compruebaFecha(int dia, int mes, int anyo){
    ...
    case MES_OCTUBRE:
    case MES_DICIEMBRE:
      if (dia > DIAS_MES_LARGO || dia < DIAS_MINIMO) {
        throw ERROR_DE_ARGUMENTOS;
      }
      break;
    ...
    case MES_SEPTIEMBRE:
    case MES_NOVIEMBRE:
      if (dia > DIAS_MES_CORTO || dia < DIAS_MINIMO) {
        throw ERROR_DE_ARGUMENTOS;
      }
      break;
    // Caso para el mes 2 (Febrero). Si es bisiesto, podemos tener 29 días. Sino, 28.
    case MES_FEBRERO:
      
      if (compruebaBisiesto(anyo)) {
        if (dia > DIAS_FEBRERO_BISIESTO || dia < DIAS_MINIMO) {
          throw ERROR_DE_ARGUMENTOS;
        }
      } else {
        if (dia > DIAS_FEBRERO_NORMAL || dia < DIAS_MINIMO) {
          throw ERROR_DE_ARGUMENTOS;
        }
      }
      ...
      break;
    default:
      // Si entramos aquí significa que el mes indicado es incorrecto
      throw ERROR_DE_ARGUMENTOS;
    }   
  }
